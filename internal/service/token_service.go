...
func ParseAccessToken(tokenStr string) (*domain2.CustomClaims, error) {
	claims := &domain2.CustomClaims{}
...
func ParseRefreshToken(tokenStr string) (*domain2.RefreshTokenClaims, error) {
	claims := &domain2.RefreshTokenClaims{}
	err := token.ParseToken(tokenStr, claims)
	if err != nil {
		tokenPreview := tokenStr
		if len(tokenStr) > 10 {
