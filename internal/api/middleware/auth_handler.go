...
func LoginRequired() gin.HandlerFunc {
	return func(client *gin.Context) {
		...
		if err != nil {
			response.Fail(client, domain2.ErrCodeTokenFormatError, err.Error())
			client.Abort()
			return
		}

		// 解析Token
		claims, err := service2.ParseAccessToken(tokenStr)
		if err != nil {
			switch err.Error() {
			case "token_expired":
				log.Printf("[登录态验证失败] Token已过期 | 请求路径：%s | Token：%s", client.FullPath(), tokenStr)
				response.Fail(client, domain2.ErrCodeTokenExpired, "登录态已过期，请重新登录")
			case "token_signature_invalid":
				log.Printf("[登录态验证失败] Token签名错误 | 请求路径：%s | Token：%s", client.FullPath(), tokenStr)
				response.Fail(client, domain2.ErrCodeTokenInvalid, "登录态无效，请重新登录")
			default:
...
func MerchantRequired() gin.HandlerFunc {
	return func(client *gin.Context) {
		...
		info, err := service2.GetUserInfoById(userID)
		if err != nil {
			log.Printf("[商户权限验证失败] 查询用户信息失败 | 用户ID：%d | 当前角色：%d", userID, info.Role)
			response.Fail(client, domain2.ErrCodeDBError, err.Error())
			client.Abort()
			return
		}

		if info.Role != domain2.RoleMerchant {
			log.Printf("[商户权限验证失败] 用户角色不符 | 用户ID：%d | 当前角色：%d | 所需角色：%d", userID, info.Role, domain2.RoleMerchant)
			response.Fail(client, domain2.ErrCodePermissionDenied, "您没有权限执行此操作")
			client.Abort()
			return
		}
