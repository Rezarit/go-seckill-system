...
import (
	...
	"github.com/Rezarit/go-seckill-system/service" // 引入 service 包！
)
...
func handleOrderMessage(body []byte) error {
	...
	// 获取购物车数据
	carts, err := service.GetCartItems(msg.UserID)
	if err != nil {
	...
	// 执行数据库下单操作
	orderID, err := service.ExecuteOrderCreation(msg.UserID, msg.Address, carts)
	if err != nil {
	...
	// 发送清空购物车的消息到 MQ
	err = service.SendMessage(cartMsg, "cart")
	if err != nil {
	...
	// 结果写入 Redis，供前端轮询
	err = service.Order.OrderResult(orderID, msg.UserID, redis.DefaultSessionTTL)
	if err != nil {
		return err
	}
