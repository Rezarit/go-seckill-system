...
func Register(req domain.UserRegisterRequest) (*domain.User, error) {
	...
	if err := CheckUsername(req.Username); err != nil {
		log.Printf("[用户注册-参数校验失败] 用户名：%s，原因：%v", req.Username, err)
		return nil, err
	}
	// 校验密码
	...
	if err := CheckUsernameExists(req.Username); err != nil {
		log.Printf("[用户注册-用户名查重失败] 用户名：%s，原因：%v", req.Username, err)
		return nil, err
	}
	...
	if err != nil {
		log.Printf("[用户注册-密码加密失败] | 用户名：%s | 错误：%v", req.Username, err)
	...
	if err = InsertUser(user); err != nil {
		log.Printf("[用户注册-数据入库失败] | 用户名：%s | 入库错误：%v", req.Username, err)
		return nil, &domain.BusinessError{
...
func InsertUser(user *domain.User) error {
	if err := dao.InsertUser(user); err != nil {
		log.Printf("[用户数据入库失败] | 用户名：%s | 入库错误：%v", user.Username, err)
		return err
	}
	log.Printf("[用户数据入库成功] 用户名：%s", user.Username)
	return nil
}

// Login 登录
func Login(req domain.UserLoginRequest) (string, string, error) {
	// 校验用户名
	if err := CheckUsername(req.Username); err != nil {
		log.Printf("[用户登录-参数校验失败] 用户名：%s，原因：%v", req.Username, err)
		return "", "", err
	}
	// 校验密码
	if err := CheckPassword(req.Password); err != nil {
	...
	if err != nil {
		log.Printf("[用户登录-数据库密码获取失败] | 用户名：%s | 错误：%v", req.Username, err)
		return "", "", err
	}
	// 核对密码
	err = ComparePassword(hashedPassword, req.Password)
	if err != nil {
		log.Printf("[用户登录-密码核对失败] | 用户名：%s | 错误：%v", req.Username, err)
		return "", "", err
	}
	...
	if err != nil {
		log.Printf("[用户登录-ID查询失败] | 用户名：%s | 错误：%v", req.Username, err)
		return "", "", err
	}
	...
	if err != nil {
		log.Printf("[AccessToken密钥转换失败] 用户id：%d，错误：%v", userID, err)
		return "", "", err
	}
	refreshToken, err := GenerateRefreshToken(userID)
	if err != nil {
		log.Printf("[RefreshToken密钥转换失败] 用户id：%d，错误：%v", userID, err)
		return "", "", err
	}
...
func GetPasswordByUsername(username string) (string, error) {
	...
	if err != nil {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			log.Printf("[密码查询失败] | 用户名：%s | 错误：用户不存在", username)
		...
		// 处理其他数据库错误
		log.Printf("[密码查询失败] | 用户名：%s | 错误：%v", username, err)
		return "", &domain.BusinessError{
			Code: domain.ErrCodeDBError,
			Msg:  "密码查询失败，请稍后再试",
		}
...
func GetUserIDByUsername(username string) (int64, error) {
	...
	if err != nil {
		// 仅处理“没找到用户”的错误
		if errors.Is(err, gorm.ErrRecordNotFound) {
			log.Printf("[ID查询失败] | 用户名：%s | 错误：用户不存在", username)
		...
		// 处理其他数据库错误
		log.Printf("[ID查询失败] | 用户名：%s | 错误：%v", username, err)
		return 0, &domain.BusinessError{
			Code: domain.ErrCodeDBError,
			Msg:  "ID查询失败，请稍后再试",
		}
...
func ComparePassword(hashedPassword, password string) error {
	...
	if err != nil {
		if errors.Is(err, bcrypt.ErrMismatchedHashAndPassword) {
			log.Printf("[密码核对失败] 错误：密码与哈希值不匹配")
			return &domain.BusinessError{
...
func CheckUsernameExists(username string) error {
	...
	if err != nil {
		log.Printf("[用户名查重失败] | 用户名：%s | 入库错误：%v", username, err)
		return &domain.BusinessError{
			Code: domain.ErrCodeDBError,
			Msg:  "服务器出现问题，请稍后再试",
		}
...
func CheckAuthHeader(authHeader string) error {
	...
	if err != nil {
		log.Printf("[Token刷新失败] 错误:传入token为空")
		return &domain.BusinessError{
...
func RefreshAccessToken(authHeader string) (string, error) {
	...
	if err != nil {
		log.Printf("[AT刷新-参数校验失败] | 原因：%v", err)
		return "", err
	}

	tokenString, err := GetTokenFromAuthHeader(authHeader)
	if err != nil {
		log.Printf("[AT刷新-RT获取失败] | 原因：%v", err)
		return "", err
	}

	parsedToken, err := ParseRefreshToken(tokenString)
	if err != nil {
		log.Printf("[AT刷新-RT解析失败] | 原因：%v", err)
		return "", err
	}

	err = ValidateRefreshToken(parsedToken)
	if err != nil {
		log.Printf("[AT刷新-RT校验失败] | 用户ID：%d | 原因：%v", parsedToken.UserID, err)
		return "", err
	}

	accessToken, err := GenerateAccessToken(parsedToken.UserID)
	if err != nil {
		log.Printf("[AT刷新-AT生成失败] | 用户ID：%d | 原因：%v", parsedToken.UserID, err)
		return "", err
	}
...
func UpdateUserPassword(userName, oldPassword, newPassword string) error {
	...
	if err != nil {
		log.Printf("[更新密码—密码获取失败] | 用户名：%s | 错误：%v", userName, err)
		return err
	}
	...
	if err != nil {
		log.Printf("[更新密码—旧密码校验失败] | 用户名：%s | 错误：%v", userName, err)
		return err
	}
	...
	if err != nil {
		log.Printf("[更新密码—新密码加密失败] | 用户名：%s | 错误：%v", userName, err)
		return err
	}
	...
	userID, err := dao.GetUserIDByUsername(userName)
	if err != nil {
		log.Printf("[更新密码—用户ID获取失败] | 用户名：%s | 错误：%v", userName, err)
		return err
	}
	...
	if err != nil {
		log.Printf("[更新密码—新密码入库失败] | 用户名：%s | 错误：%v", userName, err)
		return err
	}
...
func UpdateUserInfoByID(userID int64, user domain.User) error {
	...
	if err != nil {
		log.Printf("[更新用户信息失败] | 用户ID：%d | 错误：%v", userID, err)
		return err
	}
	return nil
}
...
func GetUserInfoById(userID int64) (domain.GetUserInfoResponse, error) {
	...
	if err != nil {
		log.Printf("[获取用户信息失败] | 用户ID：%d | 错误：%v", userID, err)
